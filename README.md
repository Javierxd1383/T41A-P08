...
### Ejemplo de Creación de Tablas e Índices (SQL)

```sql
-- 1. Crear una tabla (Entidad Estudiante)
CREATE TABLE Estudiantes (
    EstudianteID INT PRIMARY KEY,              -- Clave Primaria (por defecto, a menudo Clustered)
    Nombre VARCHAR(100) NOT NULL,
    Apellido VARCHAR(100) NOT NULL,
    Email VARCHAR(150) UNIQUE,                 -- Restricción de Unicidad
    FechaNacimiento DATE
);

-- 2. Crear un índice Non-Clustered para optimizar búsquedas por Apellido
CREATE INDEX IX_Estudiantes_Apellido
ON Estudiantes (Apellido);

-- 3. Crear una tabla relacionada (Entidad Cursos)
CREATE TABLE Cursos (
    CursoID INT PRIMARY KEY,
    NombreCurso VARCHAR(100) NOT NULL
);

-- 4. Crear una tabla de relación (Inscripciones) con una Clave Foránea
...
-- 5. Se recomienda indexar la Clave Foránea para optimizar el JOIN
CREATE INDEX IX_Inscripciones_EstudianteID
